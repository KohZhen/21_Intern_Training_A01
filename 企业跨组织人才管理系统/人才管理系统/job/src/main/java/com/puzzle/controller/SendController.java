...
public class SendController {
    ...
    public Result create(@RequestBody Map<String,Integer> map){
        ...
        sendParam.setApplicantId(userData.getId());
        ...
        send.setStatus(0);
        send.setApplicantId(userData.getId());

        //3、判断简历是否开放
        Resume param = new Resume();
            param.setApplicantId(userData.getId());
        param.setStatus(1);
    ...
    @PostMapping("applicant_send")
    public Map<String,Object> applicant_send(@RequestBody  Send send){
        String token = UserThreadLocal.get();
        UserData userData = (UserData) redisUtil.get(token);
        send.setApplicantId(userData.getId());
        PageInfo<Send> pageInfo = sendService.query(send);
